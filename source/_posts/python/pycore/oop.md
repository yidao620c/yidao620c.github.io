...
第一个`__init__`是实例初始化会调用的方法，第二个自定义方法。
...
1. 单下划线`_`表示方法是私有方法或属性，不建议直接访问，PEP8会告警
2. 双下划线`__`表示final方法或属性防止被子类覆盖，Python 会自动将 `__attr` 改写为 `_ClassName__attr`，可用来定义私有属性和方法
3. 前后双下划线比如`__init__`是python内部特殊的名字，自己没事就别定义这样的东西了

不管是模块还是类，要实现私有方法或属性，除非你有明确理由需要防止子类覆盖，否则一律用单下划线。

**何时可以考虑双下划线？**
- 你正在编写一个会被大量第三方子类化的框架/库，且某些属性极易被意外重名（例如 `__update` 这种通用名称）。
- 需要实现特殊方法不冲突（如 `__eq__`、`__hash__`，这些是双下划线开头结尾的协议方法，不是 private 用途）。
